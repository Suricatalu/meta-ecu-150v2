...
#
# poky (walnascar / 5.2) pins pseudo at SRCREV 28dcefb (PV 1.9.0+git), which
# predates pseudo's openat2 wrapper (first shipped in tag pseudo-1.9.4,
# 2026-04-20). Modern host GNU tar that carries the CVE-2025-45582 security
...
# for THIS build only, leaving poky untouched.
#
# See docs/0062_adaptive_update_via_streaming/issue_rauc_do_pakcage_pseudo_error.md
# ---------------------------------------------------------------------------

# The exact base SRCREV this workaround is meant to patch over. Guarding on
# this makes the whole bbappend SELF-DISABLING: the moment poky bumps pseudo
...
python () {
    ...
    if d.getVar('SRCREV') != known_bad:
        # poky has moved pseudo forward -> assume it now has the openat2
        # wrapper (>=1.9.4). Do nothing and remind the maintainer to drop
        # this bbappend after confirming the base pseudo is fixed.
        bb.warn("meta-ecu150v2: base pseudo SRCREV is no longer %s; assuming "
                "poky bumped pseudo to a version that already has the openat2 "
    ...
    # poky's two source-level patches do NOT apply cleanly at 1.9.8:
    ...
    if os.path.exists(configure):
        ...
        if pie_line not in cfg:
            with open(configure, 'a') as f:
                f.write("\n# meta-ecu150v2: re-apply pruned PIE flags "
